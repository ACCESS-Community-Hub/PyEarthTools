...
class PhyCell(nn.Module):
    ...
    def forward(
        ...
        if first_timestep:
            self.initHidden(batch_size, input_)  # init Hidden at each forward start
    ...
    def initHidden(self, batch_size, input_):
        self.H = []
...
class ConvLSTM(nn.Module):
    def __init__(self, input_shape, input_dim, hidden_dims, n_layers, kernel_size):
        ...
        for i in range(0, self.n_layers):
            ...
            # logging.info("ConvLSTM: layer {} input dim {} hidden dim {}".format(i, cur_input_dim, self.hidden_dims[i]))
            cell_list.append(
                ConvLSTM_Cell(
...
VALID_ACTIVATION = ["tanh", "sigmoid", None]


class EncoderRNN(torch.nn.Module):
    def __init__(self, phycell, convcell, activation="sigmoid", num_channels=1):
        ...
        self.encoder_E = self.encoder_E
        self.encoder_Ep = self.encoder_Ep
        self.encoder_Er = self.encoder_Er
        self.decoder_Dp = self.decoder_Dp
        self.decoder_Dr = self.decoder_Dr
        self.decoder_D = self.decoder_D
        self.phycell = phycell
        self.convcell = convcell

        if activation not in VALID_ACTIVATION:
            raise ValueError(f"'{activation}' not valid")
    ...
    def forward(self, input, first_timestep=False, decoding=False):
        ...
        out_phys = self.decoder_D(
            decoded_Dp
        )  # partial reconstructions for vizualization
        out_conv = self.decoder_D(decoded_Dr)
        output_image = self.decoder_D(concat)

        if self.activation is not None:
            activation_function = getattr(torch, self.activation, torch.sigmoid)

            out_phys = activation_function(
                out_phys
            )  # partial reconstructions for vizualization
            out_conv = activation_function(out_conv)
            output_image = activation_function(output_image)
...
def PhyDNet(
    ...
    imgsize = img_size
    phycell = PhyCell(
        input_shape=(imgsize // 4, imgsize // 4),
        input_dim=input_dim,
        F_hidden_dims=phycell_hidden,
        n_layers=phycell_layers,
        kernel_size=phycell_kernel,
    )
    convcell = ConvLSTM(
        input_shape=(imgsize // 4, imgsize // 4),
        input_dim=input_dim,
        hidden_dims=conv_hidden,
