...
from .cuboid_transformer_patterns import (
    ...
    def forward(self, x):
        ...
        if pad_h or pad_h or pad_w:
            ...
            x = _generalize_padding(
                x, pad_t, pad_w, pad_h, padding_type=self.padding_type
            )
...
def compute_cuboid_self_attention_mask(
    ...
    if pad_t > 0 or pad_h > 0 or pad_w > 0:
        ...
        data_mask = torch.ones(
            (1, T + pad_t, H + pad_h, W + pad_w, 1), dtype=torch.bool, device=device
        )
    if any(i > 0 for i in shift_size):
        if padding_type == "ignore":
            data_mask = torch.roll(
                data_mask,
    ...
    shift_mask = torch.zeros(
        (1, T + pad_t, H + pad_h, W + pad_w, 1), device=device
    )  # 1 T H W 1
    cnt = 0
    for t in (
        slice(-cuboid_size[0]),
    ...
    attn_mask = (
        shift_mask.unsqueeze(1) - shift_mask.unsqueeze(2)
    ) == 0  # num_cuboids, cuboid_volume, cuboid_volume
    if padding_type == "ignore":
...
class CuboidSelfAttentionLayer(nn.Module):
    ...
    def __init__(
        ...
        if use_relative_pos:
            ...
            relative_coords = relative_coords.permute(
                1, 2, 0
            ).contiguous()  # Bt*Bh*Bw, Bt*Bh*Bw, 3
        ...
        if self.use_global_vector:
            if self.separate_global_qkv:
                ...
                self.g2l_global_q_net = nn.Linear(
                    in_features=global_dim_ratio * dim, out_features=dim, bias=qkv_bias
                )
                self.g2l_k_net = nn.Linear(
                    in_features=dim, out_features=dim, bias=qkv_bias
                )
                self.g2l_v_net = nn.Linear(
                    in_features=dim, out_features=global_dim_ratio * dim, bias=qkv_bias
                )
    ...
    def forward(self, x, global_vectors=None):
        ...
        q, k, v = (
            ...
            qkv[1],
            qkv[2],
        )  # Each has shape (B, num_heads, num_cuboids, cuboid_volume, head_C)
        q = q * self.scale
        attn_score = q @ k.transpose(
            -2, -1
        ...
        if self.use_global_vector:
            global_head_C = (
                self.global_dim_ratio * head_C
            )  # take effect only separate_global_qkv = True
            if self.separate_global_qkv:
                l2g_q = (
            ...
            v_l_g = torch.cat(
                (
                    v,
                    l2g_global_v.expand(
                        B, self.num_heads, num_cuboids, num_global, head_C
                    ),
                ),
                dim=3,
            )
            ...
            if self.padding_type == "ignore":
                ...
                if pad_t > 0 or pad_h > 0 or pad_w > 0:
                    g2l_attn_mask = F.pad(
                        g2l_attn_mask, (0, 0, 0, pad_w, 0, pad_h, 0, pad_t)
                    )
                if any(i > 0 for i in shift_size):
            ...
            if self.use_global_self_attn:
                g2g_attn_score = g2g_global_q @ g2g_global_k.squeeze(2).transpose(
                    -2, -1
                )
                ...
                if g2l_attn_mask is not None:
                    g2all_attn_mask = F.pad(
                        g2l_attn_mask, (0, num_global), "constant", 1
                    )
                else:
            ...
            if self.use_global_vector:
                for idx, (attn, ffn) in enumerate(zip(self.attn_l, self.ffn_l)):
                    if self.checkpoint_level >= 2 and self.training:
                        x_out, global_vectors_out = checkpoint.checkpoint(
                    ...
                    if self.checkpoint_level >= 1 and self.training:
                        ...
                        if self.use_global_vector_ffn:
                            global_vectors = checkpoint.checkpoint(
...
def compute_cuboid_cross_attention_mask(
    ...
    if pad_t_mem > 0 or pad_h > 0 or pad_w > 0:
        ...
        mem_mask = torch.ones(
            (1, T_mem + pad_t_mem, H + pad_h, W + pad_w, 1),
            dtype=torch.bool,
            device=device,
        )
    if pad_t_x > 0 or pad_h > 0 or pad_w > 0:
        ...
        x_mask = torch.ones(
            (1, T_x + pad_t_x, H + pad_h, W + pad_w, 1), dtype=torch.bool, device=device
        )

    if any(i > 0 for i in shift_hw):
        if padding_type == "ignore":
            x_mask = torch.roll(
                x_mask, shifts=(-shift_hw[0], -shift_hw[1]), dims=(2, 3)
            )
            mem_mask = torch.roll(
                mem_mask, shifts=(-shift_hw[0], -shift_hw[1]), dims=(2, 3)
            )
    # (1, num_cuboids, cuboid_volume, 1)
    ...
    # Prepare mask based on index
    shift_mask = torch.zeros(
        (1, n_temporal, H + pad_h, W + pad_w, 1), device=device
    )  # 1 1 H W 1
...
class CuboidCrossAttentionLayer(nn.Module):
    ...
    def __init__(
        ...
        if use_relative_pos:
            ...
            relative_coords = (
                coords_flatten[:, :, None] - coords_flatten[:, None, :]
            )  # 3, maxT*Bh*Bw, maxT*Bh*Bw
            relative_coords = relative_coords.permute(
                1, 2, 0
            ).contiguous()  # maxT*Bh*Bw, maxT*Bh*Bw, 3
            relative_coords[:, :, 0] += (
                max_temporal_relative - 1
    ...
    def forward(self, x, mem, mem_global_vectors=None):
        ...
        if any(i > 0 for i in shift_hw):
            ...
            shifted_mem = torch.roll(
                mem, shifts=(-shift_hw[0], -shift_hw[1]), dims=(2, 3)
            )
        else:
            shifted_x = x
            shifted_mem = mem
        ...
        assert num_cuboids_mem == num_cuboids, (
            f"Number of cuboids do not match. num_cuboids={num_cuboids},"
        ...
        k, v = (
            kv[0],
            kv[1],
        )  # Each has shape (B, num_heads, num_cuboids, mem_cuboid_volume, head_C)
        q = (
        ...
        if self.use_global_vector:
            if self.separate_global_qkv:
                ...
                l2g_global_k, l2g_global_v = (
                    kv_global[0],
                    kv_global[1],
                )  # Shape (B, num_heads, 1, num_global, head_C)
                l2g_q = q
            l2g_attn_score = l2g_q @ l2g_global_k.transpose(
                -2, -1
...
class DownSampling3D(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.arch_type == "3d_interp_2d":
            x = self.act(
                self.inter_conv(x.permute(0, 4, 1, 2, 3))
            )  # Shape(B, mid_dim, T, H, W)
            if self.original_size[0] == self.target_size[0]:
                # Use 2D interpolation
                x = F.interpolate(
                    x.permute(0, 2, 1, 3, 4).reshape(B * T, self.mid_dim, H, W),
            ...
            x = self.act(
                self.inter_conv(x.permute(0, 1, 4, 2, 3).reshape(B * T, C_in, H, W))
            )  # (B * T, mid_dim, H, W)

            if self.original_size[0] == self.target_size[0]:
                # Use 2D interpolation
                x = F.interpolate(
                    x, size=self.target_size[1:]
                )  # Shape (B * T_new, mid_dim, H_new, W_new)
            else:
                # Use 3D interpolation
                x = F.interpolate(
                    x.reshape(B, T, C_in, H, W).permute(0, 2, 1, 3, 4),
        ...
        for i in range(self.num_blocks - 1, -1, -1):
            ...
            if not self.use_first_self_attn and i == self.num_blocks - 1:
                ...
                for idx in range(self.depth[i] - 1):
                    if (
                        self.use_self_global
                    ):  # in this case `mem_global_vector` is guaranteed to be not None
    ...
    def get_initial_z(self, final_mem, T_out):
        ...
        if self.z_init_method == "zeros":
            ...
            initial_z = torch.zeros(
                z_shape, dtype=final_mem.dtype, device=final_mem.device
            )
            ...
            initial_z = F.interpolate(
                final_mem.permute(0, 4, 1, 2, 3),
                size=(T_out, final_mem.shape[2], final_mem.shape[3]),
            ).permute(0, 2, 3, 4, 1)
            initial_z = self.z_proj(initial_z)
        elif self.z_init_method == "last":
            initial_z = torch.broadcast_to(
                final_mem[:, -1:, :, :, :], (B, T_out) + final_mem.shape[2:]
            )
            initial_z = self.z_proj(initial_z)
        elif self.z_init_method == "mean":
            initial_z = torch.broadcast_to(
                final_mem.mean(axis=1, keepdims=True), (B, T_out) + final_mem.shape[2:]
            )
            initial_z = self.z_proj(initial_z)
        else:
            raise NotImplementedError
