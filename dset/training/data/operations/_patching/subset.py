...
def cut_center(data: np.ndarray, size: Union[int, tuple[int, int]]) -> np.ndarray:
    ...
    center_values = (data.shape[-2] // 2, data.shape[-1] // 2)
    ...
    data = data[
        ...,
        center_values[0]
        - math.floor(trim_offset[0]) : center_values[0]
        + math.ceil(trim_offset[0]),
        center_values[1]
        - math.floor(trim_offset[1]) : center_values[1]
        + math.ceil(trim_offset[1]),
    ]
