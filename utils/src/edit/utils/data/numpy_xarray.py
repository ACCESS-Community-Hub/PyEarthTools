...
class NumpyConverter:
    """
    Stateful xarray -> numpy and numpy -> xarray converter.

    This class will record records of the attributes and coordinates of the
    incoming datasets, which are then used to rebuild the numpy array.
    ...
    def _distill_dataset(self, dataset: XR_OBJECT) -> dict[DISTILL_KEYS, Any]:
        ...
        for coord in dataset.coords:
            ...
            try:
                ...
                raise RuntimeError(
                    f"Cannot record coordinate, currently converter can only handle datasets with variables of the same dimensions."
                ) from e
    ...
    def _set_records(self, datasets: tuple[XR_OBJECT] | list[XR_OBJECT] | XR_OBJECT, replace: bool = False) -> None:
    ...
    def _rebuild_arrays(self, numpy_array: np.ndarray, xarray_distill: dict) -> xr.Dataset:
        ...
        if not numpy_array.shape == tuple(xarray_distill["shape"]):
            warnings.warn(
                f"Incoming and expected shape don't match. {numpy_array.shape} != {xarray_distill['shape']}.",
                RuntimeWarning,
            )
        ...
        for i in range(numpy_array.shape[xarray_distill["dims"].index("Variables")]):
            data = np.take(numpy_array, i, axis=xarray_distill["dims"].index("Variables"))
    ...
    def convert_numpy_to_xarray(
        ...
        if isinstance(data, (tuple, list)):
            ...
            return tuple(map(self.convert_numpy_to_xarray, data))

        elif isinstance(data, (np.ndarray)):
            if len(data) == 0:
                raise RuntimeError(f"Xarray records is empty, cannot rebuild any more data")
            if pop:
    ...
    def __call__(self, data: xr.Dataset | np.ndarray | tuple) -> xr.Dataset | np.ndarray | tuple:
        ...
        if isinstance(data, (xr.Dataset, xr.DataArray)):
            return self.convert_xarray_to_numpy(data)
        return self.convert_numpy_to_xarray(data)
