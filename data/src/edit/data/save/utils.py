...
LOG = logging.getLogger("edit.data.save")
...
def make_new_filename(
    path: VALID_PATH_TYPES, *, add_uuid: bool = False, prefix: str = ".tmp", remove_suffix: bool = False
) -> VALID_PATH_TYPES:
    """
    ...
    """
    if isinstance(path, list):
        return list(map(lambda x: make_new_filename(x, add_uuid=add_uuid, prefix=prefix, remove_suffix=remove_suffix), path))  # type: ignore
...
class ManageFiles:
    ...
    def __init__(
        ...
        *,
        ...
        if lock and uuid:
            raise ValueError(f"Cannot both lock files, and add uuid.")
        ...
        self._locks: list[FileLock] = [FileLock(l, timeout=timeout, thread_local=True) for l in self._lock_file_names]
        self._lock = lock
