...
If an [index][edit.data.indexes] has called `make_catalog`, a [Catalog][edit.data.CatalogEntry] is accessible at `.catalog`,
...
class CatalogEntry:
    ...
    def __getattr__(self, attribute: str):
        ...
        if attribute in ["item_class", "catalog"]:
            raise AttributeError(f"{item_class.__class__} has no attribute: {attribute}")
        ...
        *,
        ...
        if isinstance(other, (Path, str)):
            ...
            raise AttributeError(
                ...
                "Ensure, `.make_catalog()` has been run in the `__init__` of the Index."
            )
        ...
        **kwargs,
        ...
        if isinstance(catalog_to_load, (str, Path)):
            ...
            if catalog_to_load.is_dir():
                possible_catalogs = list(catalog_to_load.glob("*.cat"))
