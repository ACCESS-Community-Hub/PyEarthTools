...
from edit.data import transform, EDITDatetime
...
from edit.data.download.cds._ERA5 import ERA5_LEVELS, ERA_PRESSURE_NAME_CHANGE, ERA_SINGLE_NAME_CHANGE
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if level is None and len(as_list(set(variables).intersection(set(ERA_PRESSURE_NAME_CHANGE.keys())))) > 0:
            level = ERA5_LEVELS
            warnings.warn(
                f"As `level` was None, this will now request all levels, which may take a long time.", EDITDataWarning
            )

        self._level = level
        base_transform = TransformCollection() + transforms
        _download_transforms = TransformCollection() + download_transforms
        ...
        if level is not None and len(as_list(level)) == 1:
            ...
            #     EDITDataWarning
            #     )
            _download_transforms += transform.coordinates.assign(level=as_list(level))
        ...
        super().__init__(
            ...
            data_interval=CDS_RESOLUTION["ensemble" if "ensemble" in product else "reanalysis"],
