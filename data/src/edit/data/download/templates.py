...
class DownloadIndex(CachingIndex, metaclass=ABCMeta):
    ...
    def __init__(self, *args, pattern: PatternIndex | Type | str | None = "TemporalExpandedDateVariable", **kwargs):
        if not has_connection():
            warnings.warn(
                f"Runtime has no internet connection, and thus cannot download data. Will only work for cached data.",
                IndexWarning,
            )
