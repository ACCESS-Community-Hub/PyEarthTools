...
import edit.data
from edit.data.indexes import alias_arguments, check_arguments, VariableDefault, VARIABLE_DEFAULT
...
class OpenData(DownloadIndex):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.download_transforms = edit.data.transform.variables.rename_variables(
            **RENAME_DICT  # type: ignore
        ) + edit.data.transform.variables.variable_trim(
            *variables
        )  # + edit.data.transform.coordinates.drop("meanSea", "valid_time", "heightAboveGround", "entireAtmosphere", ignore_missing=True)

        if levels:
            transforms += edit.data.transform.coordinates.select(level=levels, ignore_missing=True)
