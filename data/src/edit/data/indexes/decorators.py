...
def _check_required_arguments(default: dict[str, inspect.Parameter], kwargs: dict, function_object: object):
    ...
    not_required = [inspect._ParameterKind.VAR_KEYWORD, inspect._ParameterKind.VAR_POSITIONAL]
...
def _get_default_arguments(signature: inspect.Signature):
    ...
    for key in function_arguments:
        ...
        if value.kind not in [inspect._ParameterKind.VAR_KEYWORD, inspect._ParameterKind.VAR_POSITIONAL]:
...
def check_arguments(struc: str | Path | dict[str, Any] | None = None, **valid_arguments: list[Any] | tuple[Any, ...] | str):
    """
    ...
    def internal_function(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            _check_required_arguments(parameter_values, kwargs, function_object=str(func.__qualname__).split(".")[0])
...
def deprecated_arguments(deprecation: dict[str, str | None] | str | None = None, *arg_deprecations: str, **extra_deprecations: str | None):
    """
    ...
    def internal_func(func):
        ...
        def warn_on_deprecated(*args, **kwargs):
            for depr in deprecation.keys():
                if depr in kwargs:
                    warning = deprecation[depr] or f"{depr!r} is a deprecated keyword, and may raise an error in future versions."
