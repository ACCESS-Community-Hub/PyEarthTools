...
class FolderSize(ByteSize):
    def __init__(self, files: list[str | Path] | tuple[str | Path] | None = None, folder: str | Path | None = None):
        if files is None and folder is None:
            raise ValueError(f"Either 'files' or 'folder' must be given.")
    ...
    def limit(
        self, max_size: ByteSize | str | int | float, key: Literal["modified", "created"] = "modified"
    ) -> list[Path]:
        """
        ...
        if self._file_size is None:
            raise RuntimeError(
                f"As this `ByteSize` was not initialised with a `folder` or `files`, a limit cannot be found."
            )
        ...
        files = sorted(
            self._file_size.items(), key=lambda x: time.time() - func(x[0]) if Path(x[0]).exists() else 0, reverse=True
        )
