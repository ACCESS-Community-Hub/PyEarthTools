...
def filter_files(files: list[str | Path] | tuple[str | Path]) -> list[str | Path] | tuple[str | Path]:
    """
    Filter blacklisted files out
    """
    new_files = []
    for file in files:
        if not Path(file).suffix in BLACKLISTED_EXTENSIONS:
            new_files.append(file)
    return new_files


def open_dataset(
    location: str | Path | tuple[str | Path] | list[str | Path], soft_fail: bool = False, **kwargs
    ...
    """
    kwargs["chunks"] = kwargs.pop("chunks", "auto")
    ...
    if isinstance(location, (tuple, list)):
        try:
            ...
            # return xr.open_mfdataset(filter_files(location), **kwargs)
            return open_dataset(filter_files(location), **kwargs)
    ...
    try:
        ...
        return preprocess(xr.open_dataset(location, **kwargs))
    except NotImplementedError:
        kwargs["chunks"] = None
        return open_dataset(location, **kwargs)
...
def invert_dictionary_list(dictionary: dict[Callable, list[str]]) -> dict[str, Callable]:
    return_dict = {}
...
def check_extension(files: tuple | list) -> list[str]:
    file_extens = []
...
def open_files(
    files: tuple[str | Path] | list[str | Path] | dict[str, str | Path] | str | Path, **kwargs
) -> Any | tuple[Any]:
    """
