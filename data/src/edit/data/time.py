...
RESOLUTION_COMPONENTS: list[VALID_RESOLUTIONS] = ["year", "month", "day", "hour", "minute", "second", "nanosecond"]

def multisplit(element: str, splits: tuple[str, ...]) -> list[str]:
    """
...
def find_components(time: str) -> dict[VALID_RESOLUTIONS, bool]:
    ...
    if len(split_time) > 1:
        time_components = [RESOLUTION_COMPONENTS[i + 3] for i in range(len(multisplit(split_time[1], (':', '.'))))]
    else:
        time_components = []
...
class TimeResolution:
    ...
    def __init__(
        self, value: dict[VALID_RESOLUTIONS, bool] | VALID_RESOLUTIONS | str | "EDITDatetime" | TimeResolution
    ):
        """
...
class EDITDatetime:
    ...
    def __init__(self, time: Any, *, resolution: str | TimeResolution | None = None):
        ...
        if isinstance(time, str):
            self._pandas_timestep = pd.to_datetime(time)
        elif isinstance(time, pd.Timestamp):
            self._pandas_timestep = time
    ...
    def at_resolution(
        self,
        resolution: VALID_RESOLUTIONS | EDITDatetime | TimeResolution | TimeDelta,
    ) -> EDITDatetime:
        """
...
@functools.total_ordering
class TimeDelta:
    def __new__(cls, timedelta: Any, *args):
        if args:
        ...
        if isinstance(timedelta, (list, tuple)) and len(timedelta) == 2:
            ...
            month_check = timedelta

        if month_check is not None and month_check.removesuffix("s") in ["year", "month"]:
            cls = _MonthTimeDelta
    ...
    def __init__(self, timedelta: Any, *args) -> None:
        ...
        if isinstance(timedelta, TimeDelta):
            ...
            timedelta = timedelta._timedelta # type: ignore

        if isinstance(timedelta, str):
            if timedelta in RESOLUTION_COMPONENTS:
                timedelta = (1, timedelta)
            elif len(timedelta.split(",")) == 2:
                timedelta = tuple(x.strip() for x in timedelta.split(","))
            elif len(timedelta.split(" ")) == 2:
                timedelta = tuple(x.strip() for x in timedelta.split(" "))
...
class _MonthTimeDelta(TimeDelta):
    ...
    def __init__(self, timedelta: Any, *args):
        ...
        if isinstance(timedelta, str) and timedelta.removesuffix("s") in RESOLUTION_COMPONENTS:
            timedelta = (1, timedelta.removesuffix("s"))
        elif isinstance(timedelta, (list, tuple)) and len(timedelta) == 1:
            timedelta = (timedelta[0], "month")
...
def time_delta(time: Any) -> pd.Timedelta | TimeDelta:
    """
...
@functools.lru_cache()
def range_samples(start: EDITDatetime, end: EDITDatetime, step: TimeDelta):
...
class TimeRange:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        !!! Warning:
            Due to the calculation and overrides in place for TimeDelta's of month's/year's the step count may be wrong,
            but the returned values and length will be correct
    ...
    def __iter__(self) -> Generator[EDITDatetime, None, None]:
        samples = range_samples(self.start, self.end, self.step)
