...
LOG = logging.getLogger("edit.data.transform.derive")
...
def _apply_equation(eq: str, dataset: xr.Dataset | None = None) -> tuple[xr.DataArray | float, list[str]]:
    ...
    # LOG.debug(f"Altered equation to: {eq!r}")  # TODO Fix Issue with 1e-6 form
...
def derive_equations(
    ...
    **equations,
    ...
    dataset = dataset.drop(drop_vars)  # type: ignore

    return dataset


def derive(
    equation: dict[str, str | tuple[str, dict[str, Any]]] | None = None, drop: bool = False, **equations
) -> Transform:
    """
    ...
    class DeriveTransform(Transform):
        """
        Derive new variables with given equations.
        """

        @property
        def _info_(self):
            return dict(**equation, drop=drop)

        def apply(self, dataset: xr.Dataset) -> xr.Dataset:
            return derive_equations(dataset, drop=drop, equation=equation)

    return DeriveTransform()
