...
from edit.data.transform.normalisation._utils import format_class_name
from edit.data.transform.transform import FunctionTransform, Transform, get_default_transforms
from edit.data.indexes.utilities.fileload import open_files
from edit.data.indexes.decorators import alias_arguments
...
def open_file(file: str | tuple | dict):
    ...
    if isinstance(data, (xr.Dataset, xr.DataArray)):
        ...
        data = edit.data.transform.coordinates.drop("time", ignore_missing=True)(data)
    return data
...
class normaliser:
    @alias_arguments(cache=["cache_dir"])
    def __init__(
        self,
        ...
        **kwargs,
        ...
        """
        warnings.warn("The transform method of normalisation will be being deprecated.", DeprecationWarning)
        ...
        **kwargs,
        ...
        save_pattern = (
            edit.data.patterns.ArgumentExpansion(
                Path(self.cache_dir) / f"{str(save_prefix)+'_' if save_prefix else ''}{method}", extension=".nc"
            )
            if self.cache_dir
            else None
        )
        ...
        aggregated_data = get_and_print(
            lambda: edit.data.transform.aggregation.over(method, dims)(
                self.index.series(
                    **retrieval_args,
                    transforms=transforms,
                ),
        ...
        if save_pattern:
            if save_pattern.exists(variable_name, *format_class_name(self.index)):
                ...
                xr.save_mfdataset(tuple(aggregate[var].to_dataset() for var in aggregate.data_vars), tuple(save_pattern.search(var) for var in aggregate.data_vars))
        ...
        if save_pattern:
            if save_pattern.search(variable_name, *format_class_name(self.index)).exists():
                ...
                for key in range.keys():
                    save_pattern.save({key: as_float(**range[key])}, key, *format_class_name(self.index))
        else:
            range = range_data()
