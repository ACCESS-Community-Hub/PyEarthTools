...
import edit.data
from edit.data import WARNING_THRESHOLD
    ...
    **kwargs,
    ...
    Args:
        ...
        transforms (Transform | TransformCollection, optional):
            Extra [Transform's][edit.data.transform.Transform] to be applied to data. Defaults to TransformCollection().
    ...
    if subset_time:
        ...
        if edit.data.EXPERIMENTAL and DataFunction.data_resolution and DataFunction.data_resolution > start.resolution:
            timesteps = [
...
def _mf_series(
    ...
    **kwargs,
        ...
        try:
            ...
            if isinstance(files, dict):
                for key, value in files.items():
                    if key not in dataset_paths:
                        ...
                        dataset_paths[key].append(value)
            else:
                key = "Data"
                if key not in dataset_paths:
                    dataset_paths[key] = [files]
                else:
                    dataset_paths[key].append(files)
            ...
            if skip_invalid:
                if warning_count < WARNING_THRESHOLD and verbose:
                    ...
                    warnings.warn(f"Warning Threshold reached, quieting warnings.", IndexWarning)
    ...
    all_paths = [i for _, l in dataset_paths.items() for i in l]
    LOG.debug(f"Opening Datasets. {all_paths}")
