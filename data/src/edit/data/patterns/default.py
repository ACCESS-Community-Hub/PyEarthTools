...
from edit.data.indexes import DataIndex, FileSystemIndex, AdvancedTimeIndex, ForecastIndex, decorators
from edit.data.save import save


class PatternIndex(DataIndex, FileSystemIndex):
    def __init__(self, *args, root_dir: str | Path | None = None, **kwargs):
...
class PatternVariableAware(PatternIndex):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(*args, **kwargs)
        self.make_catalog(override=True)
        ...
        if len(variables) == 0 and verbose:
            warnings.warn(
                f"Given variables are empty, will be unable to retrieve data from {self.root_dir!s}.", EDITDataWarning
            )
    ...
    def variable_pattern(self, variable: str) -> PatternIndex:
        ...
        init_args = list(self.catalog._args)
        init_kwargs = dict(self.catalog._kwargs)
    ...
    def _update_variables(self, new_var: str | list[str], *args):
        ...
        if hasattr(self, "catalog"):
            self.catalog._kwargs["variables"] = variables
        self.variables = variables
    ...
    def filesystem(self, *args, variables: str | list[str] | None = None, **kwargs) -> dict:
        ...
        if variables == [] or variables is None:
            raise DataNotFoundError(
                f"No variables given, therefore cannot find data. " "\nOverride '.variables' with a list of variables."
            )
