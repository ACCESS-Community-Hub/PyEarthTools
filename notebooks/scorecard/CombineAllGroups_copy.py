...
@app.cell
def _(PROCESSING_DIR, datetime, decades, filegroups, simplify, xr):
    ...
    # Let's just do 3 filegroups and 3 decades
    # for i, fg in enumerate(filegroups[3]):  # Uncomment me for testing
    for i, fg in enumerate(filegroups):
        print(f"Processing group {i} of {len(filegroups)}")
        print(datetime.now().time())
        lista = [xr.open_dataset(f, engine='h5netcdf') for f in fg]
        listb = [simplify(_ds) for _ds in lista]
        listc = xr.concat(listb, dim='report')

        for d in decades:
            decadal = listc.sel(time=slice(*d))
            if len(decadal.time):
