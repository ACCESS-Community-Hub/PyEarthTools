...
class FakeData(DataIterator):
    ...
    def __init__(
        ...
        warnings.warn(
            "Using FakeData Iterator. No data can be trusted or used for model training.",
            UserWarning,
        )
    ...
    def _get_fake_data(self, index: str | int = None) -> np.ndarray:
    ...
    def __iter__(self) -> tuple | np.ndarray:
        ...
        if not self.set_iterable and not self._start:
            raise RuntimeError(
                f"In order to iterate `num_iterations` must be given or `set_iterable` run"
            )
    ...
    def __getitem__(self, idx):
        if self.fake_data:
            return self._get_fake_index_data(index=idx)
        if isinstance(self.index, tuple):
            raise IndexError(
                f"As shape was given instead of an index, this cannot be indexed."
            )
    ...
    def __getattr__(self, key):
        if isinstance(self.index, tuple):
            raise AttributeError(
                f"As shape was given instead of an index, no further attributes can be retrieved."
            )
