...
class ToXarray(Operation):
    ...
    def __init__(self, array_shape: Union[tuple[str, ...], str], coords: Optional[dict[Hashable, Any]] = None, encoding: Optional[dict[str, Any]] = None, attributes: Optional[dict[str, Any]] = None, **kwargs):
        """
    ...
    def apply_func(self, sample: da.Array):
        ...
        if 'variable' not in self._array_shape:
            ...
            for var_i, var in enumerate(da.take(sample, i, var_index) for i in range(sample.shape[var_index])):
                ...
                data_vars[var_name] = xr.DataArray(var, coords = xarray_coord, dims = array_shape, name = var_name, attrs = ds_attrs.pop(var_name, {}))
        ...
        xr_obj = edit.data.transform.attributes.SetAttributes(self._attributes, apply_on='per_variable' if isinstance(xr_obj, xr.Dataset) else 'dataarray')(xr_obj)
        return xr_obj
    ...
    @classmethod
    def like(cls, reference_dataset: Union[xr.DataArray, xr.Dataset], drop_coords: Optional[list[str]] = None) -> 'ToXarray':
        """
