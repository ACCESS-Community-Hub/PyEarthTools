...
def gaussian(std_dev, dimensions: tuple[int] = [-2,-1]):
    ...
    def _guassian(*args):
        ...
        dist = lambda x, u: 1/(std_dev * math.sqrt(2*math.pi)) * np.power(np.e, -1/2*((x-u)/std_dev)**2) * set_at_1
...
class centre_weighted(nn.Module):
    def __init__(self, torch_loss_function: Union[Callable, str], centre_function: str = 'sin', function_kwargs: dict = {}, *, min_value: float = 0, max_value: float = 1, dimensions: tuple[int] = [-2,-1], **kwargs):
        """
