...
from .subset import cut_center, center
...
def rejoin_patches(
    ...
    # for row in patches:
    #     #rowbuild = None
    #     rowbuild = np.concatenate([cut_center(patch, size) for patch in row], axis = -1)
    #     # for patch in row:
    ...
    full_data = np.concatenate([np.concatenate([cut_center(patch, size) for patch in row], axis = -1) for row in patches], axis = -2)
...
def make_patches(
    ...
    **kwargs,
    ...
    if padding is not None:
        ...
        padd_width.append(
            [*find_dim_expand(data_as_array.shape[-2], kernel_size[0], stride[0], rounding_flip = False)]
        )
        padd_width.append(
            [*find_dim_expand(data_as_array.shape[-1], kernel_size[1], stride[1], rounding_flip = False)]
        )
