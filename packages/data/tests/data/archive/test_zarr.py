...
def test_create(tmp_path):
    zarr_archive = edit.data.archive.ZarrIndex(tmp_path / "Test.zarr")
    fake_index = edit.data.indexes.FakeIndex()
...
def test_add_to_template(tmp_path):
    zarr_archive = edit.data.archive.ZarrTimeIndex(tmp_path / "Test.zarr", template=True)
    fake_index = edit.data.indexes.FakeIndex()

    zarr_archive.make_template(
        fake_index["2000-01-01T00"],
        time=map(lambda x: x.datetime64(), edit.data.TimeRange("2000-01-01T00", "2000-01-02T00", "6 hours")),
    )
    zarr_archive.save(fake_index["2000-01-01T00"])
...
def test_combine_two_steps_exists(tmp_path):
    zarr_archive = edit.data.archive.ZarrTimeIndex(tmp_path / "Test.zarr")
    fake_index = edit.data.indexes.FakeIndex()
